...
def backup(timeStr, args, upper_policy=None):
    ...
    files_to_copy = [
        'attention_model.py', 'distributions.py', 'envs.py', 'evaluation.py',
        'evaluation_tools.py', 'givenData.py', 'graph_encoder.py', 'kfac.py',
        'main.py', 'model.py', 'storage.py', 'tools.py', 'train_tools.py'
    ]
...
def get_args():
    ...
    # 🟢 FAST DEBUGGING SETTINGS (Override defaults here)
    ...
    parser.add_argument('--use-acktr', type=bool, default=True)
    parser.add_argument('--num-processes', type=int, default=2)  # 🟢 reduced from 64
    parser.add_argument('--num-steps', type=int, default=5)      # 🟢 reduced from 5
    ...
    parser.add_argument('--model-save-interval', type=int, default=999999)
    parser.add_argument('--model-update-interval', type=int, default=999999)
    parser.add_argument('--model-save-path', type=str, default='./logs/experiment')
    parser.add_argument('--print-log-interval', type=int, default=1)

    parser.add_argument('--evaluate', action='store_true', default=True)  # 🟢 evaluate mode only
    parser.add_argument('--evaluation-episodes', type=int, default=5)     # 🟢 only 5 episodes
    ...
    parser.add_argument('--sample-left-bound', type=float)
    parser.add_argument('--sample-right-bound', type=float)
...
def get_args_heuristic():
    ...
    parser.add_argument('--load-dataset', action='store_true')
    parser.add_argument('--dataset-path', type=str)
    parser.add_argument('--heuristic', type=str, default='LSAH')
    args = parser.parse_args()
    ...
    if args.continuous:
        assert args.heuristic in ['LSAH', 'OnlineBPH', 'BR'], 'only LSAH, OnlineBPH, and BR allowed for continuous'

    args.internal_node_length = 6 if args.setting in [1, 2] else 7
    args.normFactor = 1.0 / np.max(args.container_size)
    args.num_processes = 1
    return args
